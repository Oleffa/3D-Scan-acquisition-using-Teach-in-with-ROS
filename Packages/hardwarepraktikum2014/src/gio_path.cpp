...
int CGioController::getPathFromFile(const char* fname){
	cout << "CGioController/fname: " << fname<<endl;
  int res = path->LoadFromFile(fname);
	//cout << "getPathFromFile/res:" << endl;
	cout << "getPathFromFile/res: " << res << endl;
  if(res){
    path->initTraversal();

    this->loop_exit = path->getNext();
  }	
	//debuging zeug wegen fehler mit datei finden. wurde behoben
	//cout << "______" <<endl;
	//cout << "get Path from file: loop_exit" <<endl;	
	//cout << this->loop_exit <<endl;
	//cout << "______" <<endl;
  return res;
}
...
//wenn die Roboterposition nicht bestimmt werden kann setze alles auf null!
  if(!canDetermineRobotPosition(looped)) {
    ...
    w = 0;
    vleft = 0;
	cout << "cannot determine pos" << endl;
    vright = 0;
    return 0;
  }
