...
bool CvmcAPI::getNextRequestCommand() {
	...
	for(unsigned int i=0; i < m_Request.size(); i++) {
		...
		if(m_RequestIterator->getActiveSwitchState() == true) {			// request command have to be active
...
CStorage& CvmcAPI::useVMC() {
  ...
  uint64_t ts;
	...
	while(m_Trans.findMessage(m_ReceivedMessage) == true) {	// finds a message in received data
		///////////////////////////////////DEBUG///////////////////////////////////////////
		#ifdef VMC_DEBUG
		...
		if(m_ReceivedMessage.verifyCRC() == false) {			// verifys CRC from received message
...
bool CvmcAPI::storeMessageData(const CMessage& Message) {
	...
	switch(Message.getCommandGroup()) {
		...
		default: VMC_Errors.push_back( CError("unknown command group", "VMC_API::StoreMessageData()", Message) ); break;
	}
	return false;	
}
...
bool CvmcAPI::interpreteConfigurationResponse(const CMessage& Message) {
	...
	if(DataFrame.size() > 2) {			//at least two bytes
	...
	switch(Message.getCommand()) {
		case _MCMD_SAVE_CONFIG_: 	return( m_VMC.SaveParameters.setValue(DataFrame) ); break;
		case _MCMD_LOAD_CONFIG_: 	return( m_VMC.LoadParameters.setValue(DataFrame) ); break;
		
		case _MCMD_CONTROLLER_: 	return( m_VMC.Motor[Channel.get()].VelocityControllerState.setValue(DataFrame) ); break;
		case _MCMD_USE_PWM_: 	return( m_VMC.VelocityInputType.setValue(DataFrame) ); break;
		case _MCMD_LIMITER_: 	return( m_VMC.Motor[Channel.get()].CurrentControllerState.setValue(DataFrame) ); break;
		case _MCMD_TIMEOUT_: 	return( m_VMC.VMCTimeout.setValue(DataFrame) ); break;
		
		case _MCMD_MAX_CUR_: 	return( m_VMC.Motor[Channel.get()].MaximumCurrent.setValue(DataFrame) ); break;
		case _MCMD_MAX_RPM_: 	return( m_VMC.Motor[Channel.get()].MaximumRPM.setValue(DataFrame) ); break;
		case _MCMD_NOM_CUR_: 	return( m_VMC.Motor[Channel.get()].NominalCurrent.setValue(DataFrame) ); break;
		case _MCMD_NOM_RPM_: 	return( m_VMC.Motor[Channel.get()].NominalRPM.setValue(DataFrame) ); break;

		case _MCMD_GEAR_RATIO_: 	return( m_VMC.Motor[Channel.get()].GearRatio.setValue(DataFrame) ); break;
		case _MCMD_WHEEL_RADIUS_: 	return( m_VMC.Motor[Channel.get()].WheelRadius.setValue(DataFrame) ); break;
//		case _MCMD_AXE_LENGTH_:
		case _MCMD_TICKS_ROT_: 	return( m_VMC.Motor[Channel.get()].EncoderTicks.setValue(DataFrame) ); break;
		case _MCMD_SPEC_TORQUE_: 	return( m_VMC.Motor[Channel.get()].TorqueConstant.setValue(DataFrame) ); break;
		case _MCMD_DIRECTION_: 	return( m_VMC.Motor[Channel.get()].MotorState.setValue(DataFrame) ); break;

		case _MCMD_MAX_TEMP_: 	return( m_VMC.Motor[Channel.get()].MaximumTemperature.setValue(DataFrame) ); break;
		case _MCMD_NOM_TEMP_: 	return( m_VMC.Motor[Channel.get()].NominalTemperature.setValue(DataFrame) ); break;
		case _MCMD_ENV_TEMP_: 	return( m_VMC.Motor[Channel.get()].AmbientTemperature.setValue(DataFrame) ); break;
		case _MCMD_WINDING_T_: 	return( m_VMC.Motor[Channel.get()].ThermalTimeConstantWinding.setValue(DataFrame) ); break;
		case _MCMD_WINDING_GN_: 	return( m_VMC.Motor[Channel.get()].ThermalResistanceWindingDenominator.setValue(DataFrame) ); break;
		case _MCMD_WINDING_GZ_: 	return( m_VMC.Motor[Channel.get()].ThermalResistanceWindingNumerator.setValue(DataFrame) ); break;
		case _MCMD_CHASSIS_T_: 	return( m_VMC.Motor[Channel.get()].ThermalTimeConstantMotor.setValue(DataFrame) ); break;
		case _MCMD_CHASSIS_GN_: 	return( m_VMC.Motor[Channel.get()].ThermalResistanceHousingDenominator.setValue(DataFrame) ); break;
		case _MCMD_CHASSIS_GZ_: 	return( m_VMC.Motor[Channel.get()].ThermalResistanceHousingNumerator.setValue(DataFrame) ); break;
...
//		case _MCMD_CLEAR_ALL_TICKS_ABS_: 	return( m_VMC.Motor[Channel.get()].ClearAbsolutRotations.setValue(DataFrame) ); break;
		...
		default: VMC_Errors.push_back( CError("unknown command", "VMC_API::InterpreteConfigurationResponse()", Message) ); break;
	}
...
bool CvmcAPI::interpreteStatusResponse(const CMessage& Message) {
	...
	for(unsigned int u=0; u < m_Request.size(); u++) {			// finds request buildup which fits the received one 
...
bool CvmcAPI::interpreteControllerConfigurationResponse(const CMessage& Message) {
	...
	switch(Message.getCommand()) {
		...
		default: VMC_Errors.push_back( CError("unknown command", "VMC_API::InterpreteControllerConfigurationResponse()", Message) ); break;
	}
