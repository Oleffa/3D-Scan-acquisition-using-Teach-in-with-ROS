// $Id: binary.hpp 468 2010-08-26 14:40:59Z rs $
...
>
{
    ...
    void operator=(const T& t)
    {
        if (std::numeric_limits<T>::is_iec559
            ...
            if (data_bit)
                write_little_end<uintmax_t, BASE::digits>
                    (reinterpret_cast<const uintmax_t&>(t)
                        , data, data_bit+offset);
            else
                write_little_end<uintmax_t, BASE::digits, offset>
                    (reinterpret_cast<const uintmax_t&>(t), data);
#endif
        }
    }
...
{ T t; in >> t; arg = t; return in; }

template<class charT, class traits, class BASE, unsigned offset, class It>
std::basic_istream<charT, traits>&
operator>>(std::basic_istream<charT, traits>& in, field<uint8_t,BASE,offset,It> arg)
{ unsigned int t; in >> t; arg = t; return in; }

template<class charT, class traits, class BASE, unsigned offset, class It>
std::basic_istream<charT, traits>&
operator>>(std::basic_istream<charT, traits>& in, field<int8_t,BASE,offset,It> arg)
{ int t; in >> t; arg = t; return in; }
...
{ T t =arg ; out << t; return out; }

template<class charT, class traits, class BASE, unsigned offset, class It>
std::basic_ostream<charT, traits>&
operator<<(std::basic_ostream<charT, traits>& out, field<uint8_t,BASE,offset,It> arg)
{ unsigned int t = arg; out << t; return out; }

template<class charT, class traits, class BASE, unsigned offset, class It>
std::basic_ostream<charT, traits>&
operator<<(std::basic_ostream<charT, traits>& out, field<int8_t,BASE,offset,It> arg)
...
struct sequence
{
    ...
    { if (!((end-begin)*BYTE_digits >= signed(offset)))
        throw(std::range_error("Buffer too small for sequence."));
      size_ = max_size_;
