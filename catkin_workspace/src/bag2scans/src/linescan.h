...
//typedef pair<unsigned short int, double **>
//typedef map<unsigned int, unsigned short int*> Mcorr;

typedef pair<double *, double *> lppair;
typedef vector<lppair> vlppair;
typedef map<unsigned int, vlppair > Mcorr;
//typedef map<unsigned int, vector<double *> > Mcorr;
...
public:
  ...
  bool readFromFile(int file_nr, string &dir, PointType &pointtype);
...
public:
  ...
  //LScan(vector<LineScan *> &scans, unsigned int rep);
  ~LScan();
