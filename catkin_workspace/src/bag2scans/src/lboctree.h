...
public:
  ...
  LBOctTree(vector<LineScan*> &linescans, T voxelSize ) {
    ...
    unsigned int cn = 0;
    for (int j = 0; j < linescans.size(); j++) {
    ...
    for (unsigned char i = 0; i < 8; i++) {
      this->childcenter(BOctTree<T>::center, newcenter[i], BOctTree<T>::size, i);
    }
    // set up values
    BOctTree<T>::uroot = new bitunion<T>();
    BOctTree<T>::root = &BOctTree<T>::uroot->node;

    this->countPointsAndQueueFast(pts, n, newcenter, sizeNew, *BOctTree<T>::root, BOctTree<T>::center);
    BOctTree<T>::init();
...
//      if ( points[3] >= lparams[threadNum].begin2 && points[3] <= lparams[threadNum].end2 ) 
//      if ( ( points[3] >= lparams[threadNum].begin1 && points[3] <= lparams[threadNum].end1 ) )        
//      if (true)
      {
        double myd2 = Dist2(lparams[threadNum].p, points); 
        if (myd2 < lparams[threadNum].closest_d2) {
//        cout << points[3] << " " << lparams[threadNum].begin1 <<  " " << lparams[threadNum].end1 <<  " " << lparams[threadNum].begin2 <<  " " << lparams[threadNum].end2 << endl; 
...
//    for (unsigned int i = 0; i < l->nrpts; i++) {
    for (unsigned int i = 0; i < l->nrQuery(); i++) {
      // TODO implement without copies of the points
      double *query = new double[3]; 
/*      
      query[0] = l->points[i][0];
      query[1] = l->points[i][1];
      query[2] = l->points[i][2];
      ::transform(query, l->transMat);
  */  
